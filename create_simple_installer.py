#!/usr/bin/env python3
"""
Simple Self-Extracting Installer Creator for Email Automation Bot
This creates a simple executable installer that extracts files and sets up shortcuts
"""
...
def create_installer_script():
    """Create a Python-based installer script"""
    installer_code = '''
#!/usr/bin/env python3
"""
...
'''
    ...
    with open('installer_template.py', 'w', encoding='utf-8') as f:
        f.write(installer_code)
    print("✅ Installer template created")

def create_self_extracting_installer():
    """Create a self-extracting installer by combining the script with ZIP data"""
    ...
    with open('installer_template.py', 'r', encoding='utf-8') as f:
        installer_code = f.read()
    
    # Replace the placeholder function with actual data
    zip_data_str = repr(zip_data)
    installer_code = installer_code.replace(
        'def get_embedded_data():\n    # This function will be replaced with actual ZIP data during build\n    return b""  # Placeholder',
        f'def get_embedded_data():\n    return {zip_data_str}'
    )
    
    # Write the final installer
    installer_path = 'installer_output/EmailAutomationBot_Installer.py'
    with open(installer_path, 'w', encoding='utf-8') as f:
        f.write(installer_code)
    
    print(f"✅ Self-extracting installer created: {installer_path}")
    
    # Try to create an executable version using PyInstaller
    try:
        import subprocess
        cmd = [
        ...
        if result.returncode == 0:
            # Move the executable to installer_output
            exe_src = 'dist/EmailAutomationBot_Setup.exe'
            exe_dst = 'installer_output/EmailAutomationBot_Setup.exe'
            if os.path.exists(exe_src):
                shutil.move(exe_src, exe_dst)
                file_size = os.path.getsize(exe_dst) / (1024 * 1024)
                print(f"✅ Executable installer created: {exe_dst}")
                print(f"📊 File size: {file_size:.1f} MB")
                return True
        else:
            print("⚠️  Could not create executable installer, but Python script is available")
            return True
    except Exception as e:
...
'''
    ...
    batch_path = 'installer_output/Quick_Install.bat'
...
def main():
    print("🚀 Email Automation Bot - Simple Installer Creator")
    print("=" * 50)
    
    # Check if ZIP file exists
    zip_path = 'installer_output/EmailAutomationBot_Portable_v1.0.0.zip'
    if not os.path.exists(zip_path):
        print("❌ Portable ZIP file not found. Please run create_installer.py first.")
        return False
    ...
    self_extract_success = create_self_extracting_installer()
    
    # Create batch installer
    batch_success = create_batch_installer()
    
    print("\n" + "=" * 50)
    print("✅ Simple installer packages created!")
    print("\n📁 Available installers in 'installer_output' folder:")
    
    for item in os.listdir('installer_output'):
        item_path = os.path.join('installer_output', item)
